...
async function getData(pair) {
    const select_query = 'SELECT * FROM ' + nbt_prefix + pair + ' ORDER BY eventtime DESC LIMIT ' + max_rows
    return pg_client.query(select_query)
    .then(res => {
...
async function backtest(lines) {
    ...
    for (var i in lines) {
        ...
        if (i == lines.length-1 ) { 
            ...
            if (trading) {
                const lpnl = last_price.minus(start_price).times(100).dividedBy(start_price)
                trading_report = trading_report + "\n" 
                + trading_count 
                + " " + time 
                + " " + moment(time).format() + " HODL @".magenta + price
                + " MinPNL %" + colors.red(min_pnl.toFormat(2)) 
                + " MaxPNL %" + colors.cyan(max_pnl.toFormat(2)) 
                + " CPNL %" + colors.grey(lpnl.toFormat(2)) 
                + "\n"
            }
        }
        ...
        if (
            ...
            if ( !trading ) {
                ...
                trading_report = trading_report + "\n\n" + trading_count 
                    + " " + time
                    + " " + moment(time).format() + " BUY @".blue + price 
                    + " " + colors.red(prices[prices.length-1]) 
                    + " " + colors.blue(parseInt(volumes[volumes.length-1])) 
                    + " dr:".grey + depth_report.decimalPlaces(2).toString().yellow
                    + " mk:".grey + makers_count.decimalPlaces(2).toString().yellow
                    + " si:".grey + colors.blue(srsi)
                    + " trd:".grey + colors.white(trades[trades.length-1])
                trading = true
                trading_start = time
                ...
                trading_report = trading_report + "\n" + trading_count 
                    + " " + time
                    + " " + moment(time).format() + " CONF @".cyan  + price
                    + " PNL %" + colors.yellow(pnl.toFormat(2)) 
                    + " " + colors.red(prices[prices.length-1]) 
                    + " " + colors.blue(parseInt(volumes[volumes.length-1])) 
                    + " dr:".grey + depth_report.decimalPlaces(2).toString().yellow
                    + " mk:".grey + makers_count.decimalPlaces(2).toString().yellow
            ...
            pnl = first_bid_price.minus(start_price).times(100).dividedBy(start_price)
            ...
            if (pnl.isLessThan(stop_loss)) {
                trading = false
                curr_pnl = curr_pnl.plus(pnl*(1-amount_sold)).minus(trading_fees)
                amount_sold = 0
                if (pnl.isGreaterThan(0)) {
                    ...
                    trading_report = trading_report + "\n" 
                        + trading_count 
                        + " " + time + " " 
                        + moment(time).format().grey + " SELL @".yellow + colors.grey(first_bid_price) 
                        + " MinPNL " + colors.red(min_pnl.toFormat(2))
                        + " MaxPNL " + colors.cyan(max_pnl.toFormat(2))
                        + " PNL %" + colors.yellow(pnl.toFormat(2)) 
                        + "\n"
                    ...
                    trading_report = trading_report + "\n" 
                        + trading_count 
                        + " " + time 
                        + " " + moment(time).format().red + " SELL @".red + colors.red(first_bid_price) 
                        + " MinPNL " + colors.red(min_pnl.toFormat(2))
                        + " MaxPNL " + colors.cyan(max_pnl.toFormat(2))
                        + " PNL %" + colors.red(pnl.toFormat(2)) 
                        + "\n"
                }
            ...
            if (pnl.isGreaterThan(stop_profit)) {
                trading_report = trading_report + "\n" 
                    + trading_count 
                    + " " + time 
                    + " " + moment(time).format().green + " SELL!!! @".green + colors.green(first_bid_price) 
                    + " MinPNL " + colors.red(min_pnl.toFormat(2))
                    + " MaxPNL " + colors.cyan(max_pnl.toFormat(2))
                    + " PNL %" + colors.green(pnl.toFormat(2)) 
                    + "\n"
                trading = false
                trading_win_count = trading_win_count + 1
                curr_pnl = curr_pnl.plus(pnl*(1-amount_sold)).minus(trading_fees)
    ...
    const days = (Number(lines[lines.length-1].eventtime) - Number(lines[0].eventtime)) /86400000
    console.log("-----------")
    console.log(moment().format().grey)
    console.log("-----------")
    console.log(colors.grey(days, "days"))
    console.log(moment(Number(lines[0].eventtime)).format(), 
        moment(Number(lines[0].eventtime)).fromNow(),
        lines[0].candle_close)
    console.log(moment(Number(lines[lines.length-1].eventtime)).format(), 
    ...
    console.log("HODL PnL: %".grey + last_price.minus(first_price).dividedBy(first_price).times(100).minus(0.1).toFormat(3).grey)
    console.log("Strat PnL: %".yellow + curr_pnl.toFormat(3).yellow)
    console.log("Daily PnL: ".cyan + colors.cyan(curr_pnl.dividedBy(days).decimalPlaces(3).toString().cyan) + "% /day".cyan)
    console.log("-----------")
    return true
}
////////////////////////////////////////////////////////////////////////////////////
...
async function run() {
    ...
    console.log("Ending backtest... ".green + parseInt((Date.now() - startbt)/1000) )
